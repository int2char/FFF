...
void Graph::ER(){
	int count = 0;
	map<pair<int, int>, int>flag;
	double threhod =3;// log((double)(n));
	//cout << threhod << " " << endl;
	for (int i = 0; i <this->n; i++)
		for (int j = 0; j < this->n; j++)
			if (i != j)
			{
				double ran =(double)(rand()%(n-1));// (rand() %((int)(pow(n,2)/2)));
				//cout << ran << endl;
				if (ran<threhod)
				{
...
Graph::Graph(int _n, int _m,char* file) {
	this->n = _n;
	this->m = 1000*n;
	this->incL = new Edge[m];
	this->p = new int[n + 1];
...
void Graph::FileInitial(char* file){
	FILE*rgrap = fopen(file, "r");
	int s=0, t=0;
	float c=0;
	int cnt = 0;
	while (fscanf(rgrap,"%d %d %f", &s, &t, &c) == 3)
		Graph_addEdge(s,t,1.0,c,cnt);
	this->m = cnt;
